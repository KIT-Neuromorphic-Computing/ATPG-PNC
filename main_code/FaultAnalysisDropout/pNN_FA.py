...
# from FaultAnalysisDropout.pLNC_FA import *
import random
...
class pLayer(torch.nn.Module):
    def __init__(self, n_in, n_out, args, ACT, INV):
        ...
        eta_2 = ACT.eta_fault[0, 2].detach().item()
    ...
    def DeviceCount(self):
        return self.soft_num_theta + self.soft_num_act * self.ACT.DeviceCount + self.soft_num_neg * self.INV.DeviceCount

    def MakeFaultSample(self, e_fault):
        TotalDeviceCount = [self.soft_num_theta.item(
        ), self.soft_num_act.item() * self.ACT.DeviceCount, self.soft_num_neg.item() * self.INV.DeviceCount]
        ...
        for i in fault_act:
            ...
            FaultMaskACT[i] = torch.randint(
                1, self.ACT.eta_fault.shape[0], (1,)).item()
    ...
    def generateFault(self, type_fault, indice_to_modify, fault_type_non_linear):
        '''
        This function generates the fault mask for the layer and only one type of fault is generated at a time
        indice_to_modify: An indice to modify, one of the theta, act or neg
        fault_type_non_linear: type of fault within the non-linear circuit / if type_fault is 0 then it 0-short, 1-open
        type_fault: 0 - theta, 1 - act, 2 - neg
        '''
        # type of fault: 0 - theta, 1 - act, 2 - neg
        fault_sample = [type_fault]
        ...
        flattened_mask = torch.ones(
            self.FaultMask.shape[1] * self.FaultMask.shape[2]).flatten()
        indices_to_modify = torch.tensor([indice_to_modify])[:counts[0]]
        for idx in indices_to_modify:
            flattened_mask[idx] = torch.tensor(
                10.**10., dtype=torch.int64) * fault_type_non_linear
        FaultMask = flattened_mask.reshape(
            self.FaultMask.shape[1], self.FaultMask.shape[2])

        FaultMaskACT = torch.zeros(self.theta_.shape[1])
        fault_act = torch.tensor([indice_to_modify])[:counts[1]]
        # print('check fault act', FaultMaskACT.shape,
        #       fault_act, counts[1], fault_type_non_linear)
        for i in fault_act:
            # As the non-faulty is included in eta-faults so generate eta_fault.shape[0] instead of eta_fault.shape[0]+1
            FaultMaskACT[i] = fault_type_non_linear

        FaultMaskNEG = torch.zeros(self.theta_.shape[0])
        fault_neg = torch.tensor([indice_to_modify])[:counts[2]]
        for i in fault_neg:
            # As the non-faulty is included in eta-faults so generate eta_fault.shape[0] instead of eta_fault.shape[0]+1
            FaultMaskNEG[i] = fault_type_non_linear
    ...
    def LayerMakeFault(self, e_fault, type_fault, indice_to_modify, fault_type_non_linear):
        if e_fault == 0:
            self.RemoveFault()
        else:
            FaultMask, FaultMaskACT, FaultMaskNEG = [], [], []
            faultmask, faultmaskact, faultmaskneg = self.generateFault(
                type_fault, indice_to_modify, fault_type_non_linear)
            FaultMask.append(faultmask)
            FaultMaskACT.append(faultmaskact)
            FaultMaskNEG.append(faultmaskneg)
            self.FaultMask = torch.stack(FaultMask)
...
class pNN(torch.nn.Module):
    ...
    def forward(self, x, faulty_layer_list=[], type_fault=0, indice_to_modify=None, fault_type_non_linear=None):
        '''
        ...
        self.RemoveFault()
        # self.SampleFault()
        self.MakeFault(faulty_layer_list, type_fault, indice_to_modify=indice_to_modify,
                       fault_type_non_linear=fault_type_non_linear)
    ...
    @property
    def DeviceCount(self):
        return self.soft_count_neg + self.soft_count_act + self.soft_count_theta
    ...
    def MakeFault(self, faulty_layer_list, type_fault, indice_to_modify=None, fault_type_non_linear=None):
        '''
        faulty_layer_list: list of faulty layers for initial stage it has only one element
        indice_to_modify: An indice to modify, one of the theta, act or neg
        fault_type_non_linear: type of fault within the non-linear circuit 
        type_fault: 0 - theta, 1 - act, 2 - neg
        '''
        if len(faulty_layer_list) == 0:
            self.RemoveFault()
            return
        else:
            # Indices start from 0
            counts = torch.zeros(len(self.model))
            for i in faulty_layer_list:
                counts[i] = 1

            for l, n in enumerate(counts):
                self.model[l].LayerMakeFault(
                    e_fault=n, type_fault=type_fault, indice_to_modify=indice_to_modify, fault_type_non_linear=fault_type_non_linear)
